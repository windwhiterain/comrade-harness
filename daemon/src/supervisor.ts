...
export class Supervisor {
  ...
  /** 模板 core：目录位于项目 cores/（搜索路径，CORES_DIR）之下。
   *  模板（standard/dsh-minimal）在项目 cores/ 下；fork 出的用户 core 默认在 ~/.comrade-harness/cores（项目外）。
   *  不再有 .template 标记文件——位置即语义：fork 到项目 cores/ 下的 core 也会被识别为模板。 */
  ...
  async reload(id: string): Promise<ReloadResult> {
    ...
    if (this.isTemplate(c)) {
      this.log(`[${id}] 📦 模板 core，不可重载`, id);
      return { ok: false, error: `${id} 是模板 core，不可重载（模板不是你的资产；重载由项目维护者/重启 daemon 完成）` };
    }
  ...
  async snapshot(id: string, message: string): Promise<{ ok: boolean; sha: string | null; error?: string }> {
    const c = this.cores.get(id)!;
    if (this.isTemplate(c)) {
      // 模板不是用户的资产：禁止对它 commit（要修改先 fork——fork 出的副本随便改）
      this.log(`[${id}] 📦 模板 core，禁止 commit`, id);
      return { ok: false, sha: null, error: `${id} 是模板 core，不是你的资产：禁止 commit。要修改请先 fork（fork 出的副本可以随便改、随便 commit）` };
    }
   ...
   *  源 core 不受影响（任意 core 都可以 fork 作模板来源）。
  ...
  async setUiCore(id: string): Promise<{ ok: boolean; error?: string }> {
    ...
    // 模板也可设为 UI：模板可运行（参考实现/迁移源），用户可以直接用
