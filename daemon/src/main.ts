...
// stdout 默认静默（crh web 只输出网址一行）；CRH_VERBOSE=1 恢复日志（log sink 的 WS 广播恒开）
const VERBOSE = process.env.CRH_VERBOSE === "1";
...
if (VERBOSE && PUBLIC_HOST !== "127.0.0.1") console.log(`[daemon] 局域网可达: http://${PUBLIC_HOST}:${port}（PUBLIC_HOST 可覆盖）`);
...
const sup = new Supervisor(
  ...
  (line, core) => {
    ...
    if (VERBOSE) console.log(`[${ts}] ${line}`); // CRH_VERBOSE=1 时打 stdout；WS 广播恒开
...
/** 全部 core 的 id 列表（standard/dsh-minimal 与用户 fork 的 core 完全平等）。 */
function usableCoreIds(): string[] {
  return sup.allInfo().map((c) => c.id);
}

/** 在途 agent 请求的探测：轮询旧进程 /api/status 的 busy 或 pausedSessions。
 *  旧进程还在跑 agent（聊天/自改自）或暂停中（逐步暂停挂起等继续）就等它结束再退役；
 *  404/不可达 = 立即退役。旧 core 无 pausedSessions 字段 → 只看 busy，兼容。 */
sup.retireWait = async (_id, port) => {
  try {
    const r = await fetch(`http://127.0.0.1:${port}/api/status`, { headers: authHeaders() });
    if (!r.ok) return false;
    const d = (await r.json()) as { busy?: boolean; pausedSessions?: unknown[] };
    return d.busy === true || (Array.isArray(d.pausedSessions) && d.pausedSessions.length > 0);
  } catch {
    return false;
  }
};
...
// 默认 UI core：存储的 ui_core 有效（core 存在）→ 用它；否则第一个可用 core。
// 不自动 fork——core 是用户的资产，创建由用户显式决定（壳右键卡片 fork / 导入）。
...
try {
  ...
  async fetch(req, srv) {
    ...
    // 从文件路径导入 core（直接注册 + 启动）：shell 上的「导入」按钮用，也方便 curl 测试。
    if (url.pathname === "/api/import-core" && req.method === "POST") {
      const { dir, name } = (await req.json().catch(() => ({}))) as { dir?: unknown; name?: unknown };
      if (typeof dir !== "string" || !dir.trim()) return json({ ok: false, error: "需要 dir（core 目录路径，包含 src/index.ts）" }, 400);
      const res = await sup.importCore(dir.trim(), typeof name === "string" && name.trim() ? name.trim() : undefined);
      if (res.ok) {
        // 没有 UI core 时自动把刚导入的 core 设为 UI，方便导入后立刻测试
        if (!store.getUiCore() && res.core) await sup.setUiCore(res.core.id);
        broadcast({
          type: "hello",
          protocol: PROTOCOL_VERSION,
          cores: sup.allInfo(),
    ...
    if (url.pathname === "/api/chat" && req.method === "POST") {
      ...
      // 缺省转发给当前 UI core（没有 UI core 时取第一个可用 core）
