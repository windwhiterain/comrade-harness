...
async function relayChat(text: string, coreId: string, stream: boolean, session?: string): Promise<Response> {
...
try {
  ...
  async fetch(req, srv) {
    ...
    if (url.pathname === "/api/chat" && req.method === "POST") {
      const { message, core, session } = (await req.json()) as { message?: string; core?: string; session?: unknown };
      ...
      return relayChat(message, target, wantsSSE(req), typeof session === "string" ? session : undefined);
    }
