 ...
 *   - 安装模式：bun 的 git 安装只给代码、不带 submodule——首次运行自动初始化模板子模块
...
const log = (line: string) => console.log(`[crh] ${line}`);
...
// ---- 模板子模块（standard / dsh-minimal）：fork 的来源 ----
interface Submodule {
  path: string;
  url: string;
}
...
/** 补齐模板子模块。先试 git submodule update --init（仓库检出：钉住 .gitmodules 的 gitlink 版本）；
...
async function ensureSubmodules(): Promise<boolean> {
  ...
  if (!ok)
    warn("模板子模块初始化失败——需要 git 与网络。驾驶舱仍可启动，但没有模板可 fork；重跑 crh web 会自动重试，或手动 git submodule update --init");
  return ok;
}
...
async function web() {
  ...
  log(installed ? `启动 daemon（安装模式：数据 ${DATA_DIR}，模板在包内 cores/）` : `启动 daemon（开发模式：数据 ${DATA_DIR}）`);
