...
function render() {
  ...
  // 模板 core（standard/dsh-minimal）也渲染：可见但不可使用，唯一动作是 fork（点击卡片 = fork）
  const all = [...cores.values()];
  if (all.length === 0) {
    const hint = document.createElement("li");
    hint.className = "muted";
    hint.textContent = "没有 core（右键模板卡片可 fork 一个，模板就绪时才会显示）";
  ...
  // 模板不可重载（daemon 拒绝：模板不是你的资产，重载交给项目维护者/重启 daemon）
  reloadUiBtn.disabled = !ui || ui.template || ui.status !== "healthy";
  reloadUiBtn.title = ui?.template ? "模板不可重载（不是你的资产）" : "蓝绿重载当前 UI core";
  // 模板不是用户的资产：禁止 commit（要修改先 fork）
  const sel = selectedCore ? cores.get(selectedCore) : undefined;
  commitBtn.disabled = !selectedCore || sel?.template === true;
  commitBtn.title = sel?.template ? "模板不可 commit（不是你的资产；要修改先 fork）" : "给选中 core 打 commit（git 提交，可回滚）";
}
...
function coreCard(c: CoreInfo): HTMLElement {
  ...
  if (c.template) {
    // 模板：可运行（点击 = 设为 UI 使用——参考实现/迁移源）；dirty 时警告（模板被改了，不是你的资产）
    name.textContent = c.dirty ? `📦 ${c.name} ⚠️` : `📦 ${c.name}`;
    li.title = c.dirty
      ? "📦 模板 · ⚠️ 有未提交改动（不是你的资产，会被回滚）· 点击使用"
      : "📦 模板（参考实现/迁移源）· 不要修改，要修改先 fork · 点击使用";
    li.onclick = () => {
      selectedCore = c.id;
...
function openCoreMenu(c: CoreInfo, x: number, y: number) {
  ...
  if (c.template) {
    // 模板：可运行、可 fork；不提供删除（模板是项目资产，删了由 git submodule 恢复）；重载只在按钮且模板灰掉
    item("fork", false, () => askFork(c.id, c.name));
  } else {
    // 重载只在右上角按钮（重载当前 UI core），右键不重复提供
    // fork 不改源：任意 core（含模板）都可以 fork 作来源
    item("fork", false, () => askFork(c.id, c.name));
...
function renderCommits(id: string, data: SnapshotResponse) {
  ...
  commitList.textContent = "";
  const isTpl = cores.get(id)?.template === true;
  if (isTpl) {
    // 模板不是用户的资产：顶部明示（commit 按钮已禁用）
    const notice = document.createElement("li");
    notice.className = "commit tplnotice" + (data.dirty ? " dirty" : "");
    notice.textContent = data.dirty
      ? "📦 这是模板（不是你的资产）· ⚠️ 有未提交改动——别 commit，可回滚；要修改先 fork"
      : "📦 这是模板（不是你的资产）· 不要修改，要修改先 fork";
    commitList.append(notice);
  } else {
...
// 模板 core 卡片点击与右键菜单都走 askFork（"新增 core"按钮已删除：模板卡片直接显示、点击即 fork）
