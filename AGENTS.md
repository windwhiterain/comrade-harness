...
- **多会话**（2026-08-15，路线图第 5 项）：core 的 SQLite 记忆从"单线性表"升级为**消息池（messages，只追加，内容唯一储存）+ 会话（sessions）+ 有序引用列表（session_messages）**——共享靠引用而非结构，前缀/中间/后缀消息都能被任意会话引用，fork 会话（`POST /api/sessions {name, fork}`）只复制引用、内容零复制；删除/截断/删会话都是"删引用"，消息行留在池里（孤儿，可恢复），彻底清除留给将来的 purge。**流代码零改动**：`ctx.memory` 是会话视图（`MemoryStore` 接口 = 标准消息读写，2026-08-16 起 insert 收标准消息、list 返回标准消息），runtime 按请求的 `session` 参数（缺省 `default` = 无 session 请求的兼容锚点，旧 UI/curl 行为一字不变）解析；自定义纯 `MemoryStore`（如 `sqliteMemory`）则退化为单会话（无会话 API）。端点：`GET/POST /api/sessions`、`POST /api/sessions/delete`（**运行中（busy）的会话拒绝删除**——409，flow 结束时还会往它的引用列表写消息，删了留下脏状态；先 `POST /api/abort` 再删）、`GET /api/sessions/<id>/export`（JSONL，一行一条消息含 step）、`GET/POST /api/messages?session=`、`POST /api/messages/delete|truncate|update|clear`、`POST /api/abort {session}`（不带 = 停全部）。默认对话 UI（lib ui/）有**左侧会话栏**（类似壳的 cores 列表：点击切换、右键**复制会话**（整个会话复制为新会话，`POST /api/sessions {name, fork}` 无 at = 复制全部引用，内容零复制——**消息级"分叉"已废除，复制在会话层面做**）/删除会话（default 也可删——它是"按需重建的锚点"：删光后无 session 请求会自动重建空 default 并回到列表）、＋新建（继承当前会话的 provider/model 设置）、可折叠成窄条，旧 core 无会话 API 时整栏隐藏）。**消息渲染结构（2026-08-16 UI 重构）**：一条 assistant 消息 = 一个 .msgbox 大框，内含若干 .fld 小框分装字段——reasoning / content / **每个 tool_call 一个**；LLM 统计与截断日志行（role=step）归位进 assistant 大框——**llm 统计归位到"它之后的第一个 assistant"**（DB 顺序里 flow 先落 llm 日志再落 assistant，挂"最近"会挂错到上一轮——zz-glob 实测 74/78/81 全错位；UI 用 pendingLLM 暂存，下一个 assistant 渲染时挂入，渲染完还剩下的独立一行）；cut 日志（截断/失败/已停止）在 assistant 之后或不配 assistant，仍挂最近；**空工具组框不显示**（done 类工具无 tool 结果消息——预建组框零小框，load 后清理）；响应同一 assistant 的 tool 消息们 = 另一个 .msgbox 组框（每个 tool 消息一个 .fld）；user = 单气泡框。**滚动跟随**：贴底（距底 ≤40px）时新内容自动滚到底，翻到历史则暂停跟随、滚回底部恢复；**整表重绘后恢复滚动位置**（done/删除/修改/截断都重绘）——跟随中重新钉到底，读历史时按内容高度差锚定视口顶部不拽走（`textContent = ""` 会把 scrollTop 钳到 0，不恢复 = 每次回复完成跳回顶部，真实踩过）；**布局滚动事件不误判（2026-08-16 科学修复）**：浏览器在内容增长/收缩时会自动触发 scroll 事件（不是用户动作），naive 的 scrollTop 判定会把它们误判成"用户翻走了"导致跟随失效（Theia #15822 同款症状：工具调用一出现 auto-scroll 就停）——用户滚动信号（wheel/touchmove/pointerdown/翻页键，300ms 窗口）内的滚动事件才按位置表态，无信号的布局事件在跟随中直接重新钉底；**工具调用/工具结果默认折叠（2026-08-16 用户拍板）**——长参数/长输出不撑爆页面/视口（`overflow-anchor: none` 试过无效，折叠是根治；`#msgs` 仍保留该属性作防御）；新建/切换/删除会话显式回到底部。**小框可折叠**（details 结构：标题 = summary，点击折叠/展开，高度随内容而定；菜单在 details 外，折叠时右键仍可用；**工具调用/工具结果小框默认折叠**，点击标题展开——静态与流式渲染一致）。**右键菜单**：assistant 大框 = 继续（以这条消息为最后一条**重新生成**：`POST /api/messages {at, regen: true}`，回复插入其后、后续消息保留；**有 tool_call 时不允许**——上下文终点不能是挂起工具调用的消息）/ 删除 / 截断；tool 组框 = 继续（以最后一条工具结果为终点，regen 支持 tool 结尾）/ 删除（**连带删所有对应 tool_call**）/ 截断；user = 继续 / 删除 / 截断；小框（.fld）= 删除 / 修改。**「步进」按钮（输入区，2026-08-16 合并，同日改名"步进"）**：空闲/步骤间 = 模式开关（「步进」↔「步进中」高亮，localStorage 记忆；**服务端随下次继续/发送请求的 `pause` 字段同步**——步骤间改模式对当前流不生效，下一步仍暂停，点「继续 ⏵」唤醒时同步新值，后续步骤才不再停）；暂停挂起 = 「继续 ⏵」（空继续，唤醒暂停的流）；发送中置灰。**底部「停止」按钮双态（2026-08-16）**：busy（发送中/暂停挂起）= 「停止」（终止）；空闲 = 「继续」（对最后一条消息重新生成，与右键菜单「继续」一致——2026-08-16 恢复，此前从底部移除过）。**按钮组苏联工业风配色（2026-08-16）**：军绿（发送）/卡其（步进、继续）/锈红（停止），低饱和去鲜艳，4px 小圆角。**删除联动（双向）**：assistant 侧删单个 tool_call（`clear {field:"tool_calls", toolCallId}`）连带删对应 tool 消息；tool 侧删消息（`clear {field:"tool"}`）连带移除对应 assistant 的 tool_call（copy-on-edit）——协议上 tool 消息必须挂在 assistant 的 tool_calls 下，两侧删除都保持这个不变量。**修改（原地编辑）**：点小框菜单「修改」→ 字段主体替换成文本框（预填原文），Enter 提交、Shift+Enter 换行、Esc 或失焦取消（空文本 = 取消，置空请用「删除」）——`POST /api/messages/update {id, field, text, toolCallId?}`——field ∈ content（任意 role）/ reasoning / args（改 tool_call 的 arguments，需 toolCallId）/ tool（改工具结果文本）；copy-on-edit 新建池行换引用（共享该消息的其他会话不受影响）；**不校验改得合不合法**（用户自己决定改什么）。**删除 = 置为空**（2026-08-16 交互理念：给用户最大的上下文控制——字段置空后消息保留，**即使字段全空也不删**，删除由显式按钮完成；用户始终能看到/控制上下文里有什么）。旧库自动迁移为 default 会话（旧格式行——拆行 step / 纯文本——解析失败一律跳过，不迁移，对话数据不重要）。
