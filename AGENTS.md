...
- **节点 = 普通函数**（`comrade-harness-lib/nodes.ts`）：`buildSystemPrompt`（提示词随 core 身份动态生成：standard🔒 强调不可改、fork 出的 core 强调可自改自）/ `loadHistory`（记忆：默认全量读出、不过滤不截断，显式 limit 才窗口化，step 行映射 assistant）/ `composeMessages` / `callLLM`（一次补全，失败抛出）/ `streamLLM` / `runTools`（执行 tool_calls，`done` 收尾）/ `saveHistory`。契约在 `comrade-harness-lib/types.ts`（普通 TS 接口，不是插件 API）。
