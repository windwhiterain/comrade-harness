...
- **交互理念（2026-08-16）：给用户最大的上下文控制能力**。输入区 = 输入框 + 底部一排按钮「逐步暂停 toggle / 继续 / 发送 / 停止」。**允许空消息**：发送空消息 = 一条空 user 消息（渲染「（空消息）」占位，落库进上下文，作"继续"的显式表达）；**不自动插入任何"（继续）"**——继续与否完全由用户控制（发空消息）。**删除 = 置为空**：字段级删除（思考/内容/工具）只清字段不删消息，全空也保留；整条删除由显式按钮完成（见 §4.2 消息右键菜单）。截断续跑的自动重试**不带"（继续）"轻推**（半截思考以 reasoning_content 回传后直接再请求，模型可能自己继续；连续失败到上限 giveUp 记 cut 日志，用户可手动空消息接着续）。
- **逐步暂停**（2026-08-16）：「逐步暂停」toggle（UI 本地状态，随每条消息请求体 `pause` 字段同步到 core 的**会话级开关**——暂停中也能改，关了后面的步骤不再停）。开关开启时，agent 每完成一步（该轮思考+工具执行完、结果已进上下文）就暂停：SSE 流发 `pause` 事件并挂起（暂停期间每 30s 重发一次保活，防 Bun idleTimeout 剪长静默连接；UI 幂等处理）；暂停中「继续」按钮（空 = 只继续不插入）/「发送」按钮（输入文本 = 插入消息继续）唤醒暂停门，「停止」按钮终止（abort 立即终止，回复"（已停止）"）。用户输入消息回车/点按钮 → 同一条 `/api/messages`（或 `/api/chat`）POST 唤醒暂停门（**busy 但暂停中 = 继续，不 409**；at/regen 拒绝）——文本非空则插入为一条 user 消息（落在两步之间，saveTurn 按序写库，历史与 LLM 上下文都可见）。实现：runtime 的 `pauseFor`（会话级暂停门 + abort 信号解门 + 心跳）+ `ctx.pause` 注入（agentLoop 每步收尾调用，开关没开立即返回零开销）。daemon 的 `/api/chat` 转发带 `pause` 字段透传。**坑（实测）**：bun fetch 会把新 POST 复用进未完成的 SSE 连接（Bun.serve 同连接串行处理，POST 排队等流结束）——同进程内"SSE 流未结束时再 fetch 同 core"的继续请求会挂起，测试客户端因此改 node:http；浏览器/curl 每次新连接不受影响。
