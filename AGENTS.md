...
- 可进化部分全部是**普通代码包**（`cores/` 下的目录，就是普通的 Bun/TS 服务）。**每个 core 自包含、互不依赖、完全平等**：standard 是完整 agent harness 的**参考实现**（LLM 客户端 + 工具集 + agent loop + UI 全在一个包里），与用户 fork 出的 core 没有任何区别——可改、可 reload、可 commit、可被 fork。
- 固定部分刻意最小、最笨、不可扩展：daemon + 驾驶舱外壳 + 内置 core（standard / dsh-minimal）。
- **元循环**（已实现）：`fork_core` 复制出的新 core 自带完整 harness，它的 agent 能修改任何 core——**包括运行它自己的那个**。改完自己 reload 后继续用新代码工作。这是整个设计真正想证明的事。
...
│  └─ dsh-minimal/         📦 模板 core = 极简模式活体变体（deepseek-harness minimal preset）
...
└─ 项目内 cores/dsh-minimal 是根仓库的 **git submodule**（与 standard 同形态：目录内完整 .git）。
   ...
   fork 出的用户 core 也能 fork 它。**模板只剩 📦 标识**：standard/dsh-minimal 与用户 core 行为完全平等（见 §4.4）。
```

**发布工作流（2026-08-15 定案；2026-08-16 起自动化）**：四个 GitHub 仓库（root/lib/standard/dsh-minimal 库）都压平为**单 commit v0.1.0**（tag 同名）。**后续更新一律直接 `git commit --amend` 到 v0.1.0 + force push main 与 tag**——不新建 commit、不 bump 版本。dsh-minimal core 仓库（comrade-harness-dsh-minimal）同规则。涉及 submodule 时先改并 push submodule，根仓库 amend 更新 gitlink 再 push。**amend lib 或 dsh-minimal 库后必须同步**：把新 commit sha 写进两个内置 core（cores/standard、cores/dsh-minimal）的 package.json（git 依赖是 commit id，不更新则 fork 出的 core 锁旧 id），`bun install` 刷新 bun.lock 后随内置 core 一起 amend 提交。**发布自动化**：根仓库 `bun run publish [--dry-run] [版本号]`（scripts/publish.ts）一步完成——按 lib → 内置 core → 根顺序，每个仓库自动备份分支（backup/pre-amend-<时间戳>，保住 force push 前的旧 commit）→ **只 amend HEAD（最后一条 commit），绝不 squash 历史**——想正式添加 commit 直接加，publish 不会压掉它（工作树改动并进 HEAD；干净但有未推送 commit 时直接推送不 amend）→ tag -f（v0.1.0 标记发布尖端）→ force push，只发有改动的仓库；lib 发布后自动把新 sha 同步进内置 core 的 package.json 并 install。**可选版本号**（如 `bun run publish v0.2.0`）：四个仓库（含无改动的）在各自当前 HEAD 打上新 tag 并推送——版本 tag 是发布快照标记，v0.1.0 继续作为滚动尖端。`--dry-run` 预览。
...
| `GET /api/cores` | core 列表（id/status/port/sha/dirty/template——template 仅标识，无行为限制） |
| `POST /api/reload/<id>` | 蓝绿重载，**始终等待**结果返回 `{ok,error}` |
| `POST /api/snapshot/<id>?message=` | git 快照（per-core：只提交该 core 的仓库） |
...
| `POST /api/import-core` | 从文件路径导入 core（2026-08-16）：body `{dir, name?}`，目录需包含 `src/index.ts`（没有 .git 也能运行，只是快照/回滚/fork 不可用）；直接注册 + 启动（成功且无 UI core 时自动设为 UI）；**启动失败不落库**（撤销注册，修好后重新导入）；目录已被其他 core 使用时响应带 `warning`；成功后广播 hello |
| `POST /api/set-ui/<id>` | 切换 UI core |
...
| `POST /api/chat` | 向 core 的 agent 转发消息，**同步返回最终回复**（`{ok, reply}`）；带 `Accept: text/event-stream` 则返回 SSE 流（think/tool/toolResult/delta/done，实时流出）；body `{message, core?, session?}`，core 缺省当前 UI core，session 透传给 core（core 内的会话 id，缺省 default） |
...
```
UI core 应用聊天框（页面继承自内置 core 的 public/，回落 lib 自带 ui/）→ 自己的 /api/messages → 自己的 agent loop（同一进程，不经 daemon）
...
- **会话级单飞**（2026-08-15，多会话落地；2026-08-16 暂停语义）：同一会话 busy → 409；不同会话可并行（流是纯 ctx 函数、记忆是会话视图，sqlite 语句级串行，会话内顺序由引用列表决定）。**busy = 正在执行**：逐步暂停中的会话不算忙——挂起时移出 busySessions，`/api/status` 单独暴露 `pausedSessions`（暂停中查状态不再显示 busy；继续/终止后移回）。daemon 退役轮询的 `/api/status` busy **或 pausedSessions** = 有任务在途或暂停挂起（换血等所有在途任务说完或暂停解除，上限 90s——暂停超过 90s 的换血会强杀暂停任务，先继续/abort 再 reload）。
- 退役等待：daemon 换血后轮询**旧进程** `/api/status` 的 busy **或 pausedSessions**（旧 core 无 pausedSessions 字段 → 只看 busy，兼容），等它在途 agent 任务结束或暂停解除再杀（上限 90s）——聊天中 reload 不断话，自改自（元循环）能说完。
- `relayChat` 有 600s 超时。
- **流式输出**（2026-08-15，路线图第 3 项；2026-08-16 流式进框架 + 实时落盘）：core 的 `/api/messages` 与 `/api/chat` 在请求带 `Accept: text/event-stream` 时返回 SSE 流——`think`（思考增量，deepseek-reasoner 实时上屏）/ `tool` / `toolResult`（工具调用与结果实时建卡）/ `delta`（最终回复逐字）/ `done`（收尾，携带完整回复）事件，流代码用 `ctx.emit` 发射（runtime 恒注入，JSON 模式是 no-op，流写不写都不影响）；不带 Accept 头 = 与原来完全一致的同步 JSON。`busy` 覆盖整个流（flow 结束才释放；**暂停挂起时该会话转 `pausedSessions`，退役轮询同样认**），流式聊天中 reload 依旧等说完再退役；客户端断开只停推送，flow 照常跑完（历史照存）。daemon 转发也透传 SSE（旧 core 不支持时包装成单条 `done` 事件）。**实时落盘（2026-08-16，agentLoop `live` 选项默认 true）**：每条消息产生后**立即写进 DB**（工具轮 assistant、工具结果、半截思考、最终回复、cut/llm 日志行——边生成边落，不等回合结束），流式上屏的内容与落库内容同源同构——abort/断连/刷新/重绘时已生成的内容一条不丢；**失败/中断也必须可见（2026-08-16 晚）**：LLM 调用失败、工具执行异常、工具轮中途 abort 都落一行 `cut` 日志（`error`/`aborted` 字段）并 emit——修前 catch/abort 分支只 break 不落盘，done 后重绘无变化，UI 上"回复凭空消失"（实测踩过：17 轮工具后 LLM 失败）；UI 的 cut 日志区分「错误（error 且无 finish）→ 直接显示错误文本」与「截断续跑 → 自动续跑文案」，stepBox 与流式 showCut 同文案；UI 的 error 事件分支先 load() 重绘（live 已落盘内容保留）再追加错误提示。UI 流式渲染与静态渲染同构（**每次 LLM 补全 = 一条 assistant 消息 = 一个独立大框**：思考/工具调用/正文都是可折叠小框挂进这条消息的大框，LLM 统计挂同框；工具结果们 = 独立组框紧跟这条消息——流式按事件归属：think/delta 累积进当前消息，工具轮（tool 事件）后当前消息封存，下一个补全的 think/delta 开新消息（截断同理，自动续跑 = 新消息）——**多轮"思考→工具→思考→工具"绝不累积进同一个大框**，实测踩过），done 后 load() 从 DB 整表重绘替换临时卡。**显式 `live: false` 或 `save: 函数` 回落旧行为**（循环结束后 saveTurn 一次性写）。
- **多会话**（2026-08-15，路线图第 5 项）：core 的 SQLite 记忆从"单线性表"升级为**消息池（messages，只追加，内容唯一储存）+ 会话（sessions）+ 有序引用列表（session_messages，**ord 字典序顺序键**——2026-08-16 起取代 pos 整数：追加 = 后继键 O(1) 摊还，中间插入 = 邻居间分数索引 O(键长)，键空间耗尽整表重排兜底；旧库自动迁移：ALTER 加 ord 列 + 按 pos 回填，pos 降级为唯一占位）**——共享靠引用而非结构，前缀/中间/后缀消息都能被任意会话引用，fork 会话（`POST /api/sessions {name, fork}`）只复制引用、内容零复制；删除/截断/删会话都是"删引用"，消息行留在池里（孤儿，可恢复），彻底清除留给将来的 purge。**流代码零改动**：`ctx.memory` 是会话视图（`MemoryStore` 接口 = 标准消息读写，2026-08-16 起 insert 收标准消息、list 返回标准消息），runtime 按请求的 `session` 参数（缺省 `default` = 无 session 请求的兼容锚点，旧 UI/curl 行为一字不变）解析；自定义纯 `MemoryStore`（如 `sqliteMemory`）则退化为单会话（无会话 API）。端点：`GET/POST /api/sessions`、`POST /api/sessions/delete`（**运行中（busy）的会话拒绝删除**——409，flow 结束时还会往它的引用列表写消息，删了留下脏状态；先 `POST /api/abort` 再删）、`GET /api/sessions/<id>/export`（JSONL，一行一条消息含 step）、`GET/POST /api/messages?session=`、`POST /api/messages/ops`（**单一编辑端点，2026-08-16 取代 delete/truncate/update/clear 四个特例**：`{session, ops}` 原子批量，op 词汇 insert（after 缺省 = 末尾）/ update（路径寻址，remove: true = 置空/删路径）/ remove（删消息，tool 消息联动）/ truncate（删 from 及其后）/ move（移到 after 之后）——路径段 content / reasoning_content / tool_calls / `<tcId>` / function / arguments，数组段按元素 id 匹配不按下标；任一 op 失败整体回滚）、`POST /api/abort {session}`（不带 = 停全部）。默认对话 UI（lib ui/）有**左侧会话栏**（类似壳的 cores 列表：点击切换、右键**复制会话**（整个会话复制为新会话，`POST /api/sessions {name, fork}` 无 at = 复制全部引用，内容零复制——**消息级"分叉"已废除，复制在会话层面做**）/删除会话（default 也可删——它是"按需重建的锚点"：删光后无 session 请求会自动重建空 default 并回到列表）、＋新建（继承当前会话的 provider/model 设置）、可折叠成窄条，旧 core 无会话 API 时整栏隐藏）。**消息渲染结构（2026-08-16 UI 重构；同日词汇统一；同日 steps 归位）**：一条 assistant 消息 = 一个 .msgbox 大框，内含若干 .fld 小框分装字段——reasoning_content / content / **每个 tool_call 一个**（小框 dataset.field = OpenAI 标准字段名）；**过程日志内嵌（steps 归位）**：llm 统计 / 截断诊断（cut）从独立 step 行收进所属 assistant 的 `meta.steps`（渲染随卡、删除卡片连带删除、孤儿不可能；UI 的 pendingLLM 状态机与"DB 顺序约定"整个删除——不再有"llm 行挂哪个 assistant"的错位问题）；独立 `role="step"` 行只剩两种：**旧数据**（不迁移，独立展示）与**无 assistant 可挂的失败日志**（LLM/工具失败、空回复、giveUp、终止）；**空工具组框不显示**（done 类工具无 tool 结果消息——预建组框零小框，load 后清理）；响应同一 assistant 的 tool 消息们 = 另一个 .msgbox 组框（每个 tool 消息一个 .fld）；user = 单气泡框。**大框折叠 bar（2026-08-16，取代 role 气泡）**：每条大框（assistant / user / tool 组框）顶部的原生 details/summary bar——前面显示角色名（助手/用户/工具，按角色着色），**左键折叠/展开整卡**（与小框的折叠 bar 同款交互），**右键 bar 自身不拦截**（无 contextmenu 处理，事件交给外层大框的右键菜单，openMenuFor 的 closest(".msg") 分支命中）；折叠中的卡右键会自动展开（菜单在卡内）；step 日志行与系统提示无 bar。**回复为空不显示「回复」小框（2026-08-16 用户拍板）**：静态渲染 trim 检查跳过空内容（空串/纯空白）；流式回复框**惰性创建**——第一条 delta 才建，纯工具轮/思考轮不出现空「回复」框。**滚动跟随**：贴底（距底 ≤40px）时新内容自动滚到底，翻到历史则暂停跟随、滚回底部恢复；**整表重绘后恢复滚动位置**（done/删除/修改/截断都重绘）——跟随中重新钉到底，读历史时按内容高度差锚定视口顶部不拽走（`textContent = ""` 会把 scrollTop 钳到 0，不恢复 = 每次回复完成跳回顶部，真实踩过）；**布局滚动事件不误判（2026-08-16 科学修复）**：浏览器在内容增长/收缩时会自动触发 scroll 事件（不是用户动作），naive 的 scrollTop 判定会把它们误判成"用户翻走了"导致跟随失效（Theia #15822 同款症状：工具调用一出现 auto-scroll 就停）——用户滚动信号（wheel/touchmove/pointerdown/翻页键，300ms 窗口）内的滚动事件才按位置表态，无信号的布局事件在跟随中直接重新钉底；**工具调用/工具结果默认折叠（2026-08-16 用户拍板）**——长参数/长输出不撑爆页面/视口（`overflow-anchor: none` 试过无效，折叠是根治；`#msgs` 仍保留该属性作防御）；新建/切换/删除会话显式回到底部。**小框可折叠**（details 结构：标题 = summary，点击折叠/展开，高度随内容而定；菜单在 details 外，折叠时右键仍可用；**工具调用/工具结果小框默认折叠**，点击标题展开——静态与流式渲染一致）。**右键菜单 = 单一全局菜单（2026-08-16，取代每条消息内嵌的 .actions）**：`#ctxMenu` 一个元素，openMenu(items, x, y) 按右键目标动态构建——assistant 大框 = 继续（以这条消息为最后一条**重新生成**：`POST /api/messages {at, regen: true}`，回复插入其后、后续消息保留；**有 tool_call 时不允许**——上下文终点不能是挂起工具调用的消息）/ 删除 / 截断；tool 组框 = 继续（以最后一条工具结果为终点，regen 支持 tool 结尾）/ 删除（**连带删所有对应 tool_call**）/ 截断；user = 继续 / 删除 / 截断；小框（.fld）= 删除 / 修改（语义按字段：content/reasoning_content 置空、tool_calls 删单个调用/改参数、tool 结果删消息/改结果文本）；会话项 = 复制会话 / 删除会话；右键任意位置先关旧菜单，无菜单目标（llmstat/日志行）= 只关闭。**小框/组框构建 = 单一构建器（2026-08-16 单渲染器核心）**：`fieldBox`（字段小框，静态与流式共用，流式传 m.id=null 自动标 live）与 `toolGroupBox`（工具结果组框）——卡片结构只有一份定义，新字段类型改一处即可。**「步进」按钮（输入区，2026-08-16 合并，同日改名"步进"）**：空闲/步骤间 = 模式开关（「步进」↔「步进中」高亮，localStorage 记忆；**服务端随下次继续/发送请求的 `pause` 字段同步**——步骤间改模式对当前流不生效，下一步仍暂停，点「继续 ⏵」唤醒时同步新值，后续步骤才不再停）；暂停挂起 = 「继续 ⏵」（空继续，唤醒暂停的流）；发送中置灰。**底部「停止」按钮双态（2026-08-16）**：busy（发送中/暂停挂起）= 「停止」（终止）；空闲 = 「继续」（对最后一条消息重新生成，与右键菜单「继续」一致——2026-08-16 恢复，此前从底部移除过）。**按钮组苏联工业风配色（2026-08-16）**：军绿（发送）/卡其（步进、继续）/锈红（停止），低饱和去鲜艳，4px 小圆角。**删除联动（双向）**：assistant 侧删单个 tool_call（ops `update {path:"tool_calls/<tcId>", remove: true}`）连带删对应 tool 消息；tool 侧删消息走 ops `remove`——**delete 内置联动**（删 tool 消息连带移除对应 assistant 的 tool_call，copy-on-edit）——协议上 tool 消息必须挂在 assistant 的 tool_calls 下，两侧删除都保持这个不变量。**字段词汇 = OpenAI 标准消息形状（2026-08-16 词汇统一）**：`content / reasoning_content / tool_calls`——UI 的 dataset.field、API 的 path、store 的路径参数同一套，无映射；tool 消息的字段 = `content`（改工具结果文本）；`tool_calls` 带 tcId 改/删该调用的 arguments、不带 = 清空全部。**修改（原地编辑）**：点小框菜单「修改」→ 字段主体替换成文本框（预填原文），Enter 提交、Shift+Enter 换行、Esc 或失焦取消（空文本 = 取消，置空请用「删除」）——ops `update {id, path, value}`：path ∈ content（任意 role，含工具结果文本）/ reasoning_content（改思考）/ tool_calls/<tcId>/function/arguments（改工具参数）；copy-on-edit 新建池行换引用（共享该消息的其他会话不受影响）；**不校验改得合不合法**（用户自己决定改什么）。**删除 = 置为空**（2026-08-16 交互理念：给用户最大的上下文控制——字段置空后消息保留，**即使字段全空也不删**，删除由显式按钮完成；用户始终能看到/控制上下文里有什么）。旧库自动迁移为 default 会话（旧格式行——拆行 step / 纯文本——解析失败一律跳过，不迁移，对话数据不重要）。
...
- **交互理念（2026-08-16）：给用户最大的上下文控制能力**。输入区 = 输入框 + 底部一排按钮「暂停/继续 / 发送 / 停止」（2026-08-16 晚：toggle 与继续按钮**合并成一个**——空闲/步骤间 = 「暂停/暂停中」模式开关，暂停挂起 = 「继续 ⏵」唤醒）。**允许空消息**：发送空消息 = 一条空 user 消息（渲染「（空消息）」占位，落库进上下文，作"继续"的显式表达）；**不自动插入任何"（继续）"**——继续与否完全由用户控制（发空消息）。**删除 = 置为空**：字段级删除（思考/内容/工具）只清字段不删消息，全空也保留；整条删除由显式按钮完成（见 §4.2 消息右键菜单）。截断续跑的自动重试**不带"（继续）"轻推**（半截思考以 reasoning_content 回传后直接再请求，模型可能自己继续；连续失败到上限 giveUp 记 cut 日志，用户可手动空消息接着续）。
- **逐步暂停**（2026-08-16）：「暂停/继续」合并按钮（2026-08-16：原 toggle + 继续按钮合一；UI 本地状态，随每条消息请求体 `pause` 字段同步到 core 的**会话级开关**——**步骤间（sending）可改**：关了后面的步骤不再停，当前流不感知、下次请求生效；**暂停挂起中按钮变「继续 ⏵」**，点击唤醒并同步当前模式）。开关开启时，agent 每完成一步（该轮思考+工具执行完、结果已进上下文）就暂停：SSE 流发 `pause` 事件并挂起（暂停期间每 30s 重发一次保活，防 Bun idleTimeout 剪长静默连接；UI 幂等处理）；暂停中「继续」按钮（空 = 只继续不插入）/「发送」按钮（输入文本 = 插入消息继续）唤醒暂停门，「停止」按钮终止（abort 立即终止，回复"（已停止）"）。用户输入消息回车/点按钮 → 同一条 `/api/messages`（或 `/api/chat`）POST 唤醒暂停门（**busy 但暂停中 = 继续，不 409**；at/regen 拒绝）——文本非空则插入为一条 user 消息（落在两步之间，saveTurn 按序写库，历史与 LLM 上下文都可见）。**暂停 ≠ busy（2026-08-16）**：挂起时会话移出 busySessions，`/api/status` 的 `pausedSessions` 单独列出（继续/终止后移回）——暂停时查状态是 `busy:false + pausedSessions:[sid]`，不再误导；删除会话对暂停中同样 409（继续后还会写库，先 abort 再删）。实现：runtime 的 `pauseFor`（会话级暂停门 + abort 信号解门 + 心跳）+ `ctx.pause` 注入（agentLoop 每步收尾调用，开关没开立即返回零开销）。daemon 的 `/api/chat` 转发带 `pause` 字段透传。**坑（实测）**：bun fetch 会把新 POST 复用进未完成的 SSE 连接（Bun.serve 同连接串行处理，POST 排队等流结束）——同进程内"SSE 流未结束时再 fetch 同 core"的继续请求会挂起，测试客户端因此改 node:http；浏览器/curl 每次新连接不受影响。
- **终止响应**（2026-08-15；工具可中断 2026-08-16；2026-08-16 再修"中止泄露"）：UI 的「停止」按钮（输入区底部一排，发送中/暂停中可用）→ `POST /api/abort` → core 中断当前任务的 LLM 调用（`ctx.abortSignal` 注入，`llm.ts` 的 readSSE 每轮迭代检查；Bun 的 fetch 会把已缓冲 body 继续读完，必须主动检查否则 abort 形同虚设——真实踩过）→ **半截思考落库当思考过程（UI 显示思考卡，后续空消息还能接着续），半截 content 绝不落成最终回复**：手动 abort 发生在模型思考中途时，readSSE 会把半截输出原样返回，若当回复保存就泄露成"最终回答"（真实踩过）——只记 cut(aborted) 日志行，回复标「（已停止）」。流照常以 `done` 收尾，busy 随之释放。**正在执行的工具也会被终止**（2026-08-16 修复：此前工具是原子的"跑完这轮再停"——`run_cmd` 用 `Bun.spawnSync` 同步阻塞，命令执行期间整个 core 的事件循环被冻结，`/api/abort` 根本排不上队，停止按钮形同虚设）：`run_cmd` 改异步 `Bun.spawn` + 终止时杀进程树（Windows `taskkill /T /F`），daemon 类工具（reload/snapshot/fork）的 fetch 带 signal，`runTools` 在工具轮之间检查 signal 不再调度剩余工具；持久 bash 工具（dsh-minimal）的 `execute(args, signal)` 把终止信号接进轮询循环，终止时杀掉 shell 复位（Windows 上不能用 `spawnSync taskkill` 等 close 事件——MSYS bash 被杀后 close 会迟到命令自然结束，实测；用 `child.kill()` 即时触发 + 异步 taskkill 补刀进程树）。工具结果如实记录（"（已停止）命令被终止"），回复"（已停止）"。daemon 侧 `POST /api/abort/<id>` 转发。流式代码里 LLM 调用记得传 `{ signal: ctx.abortSignal }`，工具节点传 `ctx.abortSignal` 给 `runTools`。

### 4.3 节点 + 数据流（harness lib）

**三层**：资源（LLM / 记忆 / 工具 / UI / HTTP 壳——参数组合就够了，Claude/Codex 也能"加载资源"）、**节点**（层级 0，原语）、**子图**（层级 1，数据流片段）+ **标准流**（层级 2，默认组合）。

- **节点 = 普通函数**（`comrade-harness-lib/nodes.ts`）：`buildSystemPrompt`（提示词随 core id 生成，所有 core 一致：你是这个 core 的 agent，可以自由修改自己）/ `loadHistory`（记忆：默认全量读出、不过滤不截断，显式 limit 才窗口化；step 行重建为标准 message——think → assistant 文本，tool → assistant.tool_calls + tool 角色消息对）/ `composeMessages` / `callLLM`（一次补全，失败抛出）/ `streamLLM` / `runTools`（执行 tool_calls，`done` 收尾）/ `saveHistory`。契约在 `comrade-harness-lib/types.ts`（普通 TS 接口，不是插件 API）。
- **子图 = 数据流片段**（`comrade-harness-lib/flow.ts`，2026-08-16 新增）：普通函数，用控制流把节点串成完整语义——`loadContext`（上下文子图：system + 全量历史 + 本条消息，含 regen 处理）/ `agentLoop`（LLM↔工具循环子图：SSE emit、abort「（已停止）」、步骤收集、**实时落盘**——`live` 选项默认 true，每条消息产生后立即写库；live:false 时只累积）/ `saveTurn`（记忆子图：user → step → agent 写回；standardFlow 默认走 agentLoop 实时落盘，saveTurn 只在 live:false / loop 函数 / 自定义 flow 时用）。可独立使用、自由组合。
...
- 默认工具包（`tools.ts`）：`read_file / write_file / run_cmd / snapshot / reload / rollback / fork_core / core_info / done`；`bun`/`bunx` 前缀重写成 `process.execPath`（Windows 兼容）；提示词已写明 Windows 无 ps/ss、改完必须 tsc+reload、验证用 core_info + bun -e fetch。
...
- **上下文注入**（路线图第 6 项；2026-08-16 默认改全量）：standard 的流默认**全量历史**进上下文——`loadHistory` 不过滤不截断，每次消息不再是独立回合；想窗口化就显式传 `load: { history: N }`（最近 N 条），独立回合 `history: 0`。
- **消息存储 = 严格 OpenAI 标准（2026-08-16 重构）**：一次 LLM 响应 = **一条** assistant 消息（`content` / `reasoning_content` / `tool_calls` 三字段一体，原样落库）；工具结果 = 独立 `tool` 消息（tool_call_id 对应）；用户消息（含空消息）= `user` 消息。`loadHistory` **严格重建**：存储就是标准消息，读出后原样回传 LLM，不拆不并不改写（旧设计把一次响应拆成 think/tool 多行 step、重建时再拼回多条 assistant 的做法已废除；2026-08-16 起 assistant 的 `meta`（过程日志）**剥掉**——只存不喂，绝不进模型上下文）。日志行（`role="step"` 存 JSON 的 cut/llm）只给 UI 展示（像 LLM 统计那样一行），不进 LLM 上下文；旧格式数据解析失败一律跳过（不迁移）。思考片段 = DeepSeek 推理模型的 `reasoning_content`（llm.ts 已适配；`LLM_MODEL=deepseek-reasoner` 时每轮都有思考卡），没有 reasoning 字段的模型（如 deepseek-chat）工具轮 content 通常为空。**截断续跑**：截断（finish=length/content_filter、流内 error、未收到 finish_reason）且带半截思考 → 半截思考以 assistant 的 reasoning_content 字段回传再请求（MAX_RESUME=5 防无限重试；giveUp 记 cut 日志）；**不带"（继续）"轻推**（交互理念：继续由用户控制，见 §4.2）。**abort**：半截思考落库当思考过程，半截 content 绝不落成最终回复（abort 泄露是真实踩过的坑），只记 cut(aborted) 日志，回复"（已停止）"。
...
- **UI 是 lib 的资源**（2026-08-15）：默认对话 UI（index.html/app.css/app.js 三件套）放在 lib 包内 `ui/` 目录（package.json 的 `files` 白名单含 `"ui"`，git 依赖安装时随包分发）。`createHarness` 的 `ui: { dir, shared }`：core 的 `dir`（约定 public/）**优先**，缺失文件回落 `shared`（缺省 = lib 自带 ui/，显式 `shared: null` 关闭回落）——内置 core/新 fork 的 public/ 天然是**空覆盖层**，只有变体差异才放文件（同名文件即覆盖）。UI 改一处（lib）全部 core 生效，不再跨 core 手动 cp。本地 `local:on` 时改 lib 的 ui/ 刷新即生效（静态文件按请求读盘，不用重启 daemon）。

### 4.4 模板 = 标识，不是限制（2026-08-16 定案）——所有 core 行为完全平等

**模板的所有行为限制已废除**：daemon 没有 reload/snapshot 守卫（对全部 core 放行）、壳没有禁用按钮/只 fork 菜单（commit/重载按钮对全部 core 可用，右键菜单统一提供 fork/删除）、lib 提示词没有"你是模板，别改自己"（buildSystemPrompt 对所有 core 一视同仁：可以自由修改自己）。standard / dsh-minimal 与任何用户 core 一样：可运行、可设为 UI、可修改自己、可 reload、可 commit、可删除。

**模板只剩标识**：`isTemplate()`（目录位于项目 `cores/` 搜索路径下）只用来给协议 `CoreInfo.template` 填值，壳据此在卡片上显示 `📦`（虚线边框 + title「模板 core（项目内置参考实现）」）——让用户知道这是项目内置 core。仅此而已，没有任何行为分支。两个内置 core 的 AGENTS.md 也只写"你是模板（参考实现），与任何 fork 出的 core 完全平等"，不写任何限制。

- 它们仍是根仓库的 git submodule（项目资产）：删掉后 `git submodule update --init` 可恢复；对它们 commit 只写进本 core 自己的仓库，根仓库的版本指针（gitlink）由项目维护者管理。
- 创建新 core 仍然显式：壳右键卡片 fork / `POST /api/fork` / 「导入」（没有可用 core 时不自动 fork——core 是用户的资产）。UI core 只在两个时机被隐式选择：启动时存储的 ui_core 无效则取第一个可用 core；删除当前 UI core 时回落到第一个可用 core（全都没有则置空）。`/api/chat` 缺省目标 = 当前 UI core，没有可用 core 时返回 `没有可用的 core`。
- agent 侧：工具层没有任何 immutable 硬守卫——"agent = 开发者"信任模型的彻底版，防手滑的只剩提示词，真正的兜底永远是 git（快照 + 回滚）。dsh-minimal 的 system prompt 是纯 PERSONA_TEXT（过拟合保护），连提示词都没有——只有 AGENTS.md。
...
- **允许 fork 任意 core，包括内置的 standard / dsh-minimal**（fork 不改源；新 core 默认在项目外 `~/.comrade-harness/cores/`）。**新 core 自带完整 agent harness**（提示词随 CORE_ID 生成，所有 core 一致：可以自由修改自己）。
...
- 入口两个：壳（右键菜单 fork）；agent 有 `fork_core` 工具。
...
4. **所有 core 完全平等**：standard/dsh-minimal 与用户 fork 的 core 一样可运行、可修改、可 reload、可 commit、可删除（2026-08-16 模板特判全删，见 §4.4）。它们只是根仓库的 submodule（项目资产），真正的兜底永远是 git 快照 + 回滚。
...
9. **日志流向**：daemon 的 log sink 恒广播 WS；**stdout 默认静默**（2026-08-16 起 `crh web` 只输出驾驶舱网址一行，`[HH:MM:SS]` 日志与局域网可达提示收进 `CRH_VERBOSE=1`）。壳的日志面板已删除，调试用 `CRH_VERBOSE=1 crh web` 看 daemon 的 stdout。
10. **fork 复制的是 git 内容**：`git clone` 不带工作树未提交改动。已修：fork 前自动快照源。但**先改未提交 → 立刻 fork** 会先产生一个快照提交，别惊讶。
11. **改 standard 的代码要重启 daemon**：standard 与所有 core 一样**正常 spawn**（参考实现）；改它只影响**之后** fork 出的新 core（已 fork 的 core 是独立 git 仓库，不受影响）。开发流程 = 改代码 → tsc → 重启 daemon。**改 harness 组件库也一样**（core 启动时动态 import，重启才换组件）。
...
14. **本地依赖覆盖（local.override.json + postinstall，2026-08-16 取代 package.local.json/skip-worktree）**：`bun run local:on` 生成 gitignored 的 local.override.json（依赖名 → 本地路径）并 install；package.json 的 postinstall 钩子（scripts/local-link.ts）把列出的依赖链成 junction——package.json/bun.lock **始终是提交版 git 依赖**，git pull / git reset 不再有静默覆盖问题，发布也不用 un-skip。`local:off` 删 marker 还原 GitHub 安装。**注意**：local-link.ts 是每次 install 都跑的钩子，fork/他人机器无 marker 时必须是 no-op（exit 0）；改它要保证零影响。
...
17. ~~immutable 标记实时读取~~ **已废除（2026-08-15）**：`.immutable`/`.template` 权限系统整体删除，模板识别改为搜索路径（项目 cores/ 目录）；**2026-08-16 模板限制全删、标识保留**——isTemplate 只用于 UI 显示（📦），reload/snapshot/删除与普通 core 完全平等，见 §4.4。
...
20. **权限系统已废除（2026-08-15；2026-08-16 模板只剩标识）**：`.template`/`.immutable` 标记与三层守卫整体删除——先改为搜索路径识别，后删光所有行为限制（reload/snapshot 对模板放行、壳无禁用按钮/只 fork 菜单、提示词无"你是模板"），**只保留 `isTemplate` 路径判定给 `CoreInfo.template` 填值供壳显示 📦**，所有 core 行为完全平等（可改、可 reload、可 commit、可删除），见 §4.4。**fork 不再自动快照**：直接基于源的最新提交，源有未提交修改时 daemon 响应带 `warning`、壳弹确认、agent 先 snapshot 再 fork——别再把"fork 前自动提交"加回去（2026-08-14 的旧行为，用户已拍板废除）。协议升到 v6：CoreInfo 删 `immutable`、加 `dirty`；`/api/snapshots` 返回 `{dirty, list}`。v7 起 `template` 字段含义改为「仅标识，无行为限制」。
19. **git 依赖必须用 commit id，不能用 tag**（2026-08-15，真实踩过）：内置 core（standard/dsh-minimal）的 package.json 曾提交 `github:...#v0.1.0`（tag），而发布工作流是 force amend 移动 tag——**bun 只感知 package.json 的 spec 变化，不感知 tag 移动**，bun.lock 永远锁旧 sha；fork 复制提交版 bun.lock → 新 core `bun install` 拉到旧 lib（如缺 `importProviders` 导出）→ SyntaxError 起不来，日志与报错都指向代码而非依赖。已修：依赖改为 **commit id**（不可变，package.json 与 lock 天然一致，即使复制旧 lock，install 也按 package.json 拉正确版本；已实测验证）。**发布新版本后必须把新 commit sha 同步进内置 core 的 package.json**（§2 发布工作流），否则 fork 出的 core 锁旧 id（force push 后旧 commit 不可达，重装会 404）。
...
22. **lib ui/ 的 app.js 是纯 JS，不在 tsc 覆盖内**（2026-08-16，真实踩过）：改 UI 时在同块作用域重复声明 `const pre`（addMsg 的 tool 分支：`toolCards.get` 与孤立工具结果的 `createElement` 用了同名变量）——SyntaxError 导致**整个 app.js 不执行**：事件绑定全失效（点"+"没反应）、init IIFE 不跑（会话侧栏保持 hidden，看似"会话全没了"），而服务端 /api/sessions 完全正常，极易误诊为存储/服务端问题。**改完 ui/ 先 `node --check ui/app.js` 再让用户刷浏览器**（UI 静态文件按请求读盘，改完即生效，无需重启 daemon）。
...
6. ~~**会话上下文注入**~~ ✅ **已落地（2026-08-14，随数据流化；2026-08-16 默认改全量）**：standard 的流默认全量历史进上下文（`loadContext` 不过滤不截断，标准消息原样回传）；想窗口化 `load: { history: N }`，独立回合 `history: 0`。
...
2. `curl http://127.0.0.1:3800/api/health` → 首次启动**没有可用 core**（2026-08-15 起不再自动 fork default，需手动创建）；standard 与所有 core 一样正常启动（参考实现，可直接设为 UI 使用）
3. `curl -X POST "http://127.0.0.1:3800/api/fork/standard?name=default"` → 手动创建首个 core；`curl -X POST http://127.0.0.1:3800/api/chat -d '{"message":"你好"}'` → 验证 agent 链路（缺省走当前 UI core；没有 UI core 时取第一个可用 core）
