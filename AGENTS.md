...
│  │  ├─ src/index.ts      ★ 数据流：用 lib 节点函数串成的 harness（替换/插入节点 = 改这段代码）
...
│  index.ts（入口）/ nodes.ts（节点函数）/ llm.ts / tools.ts / memory.ts / runtime.ts（HTTP 壳）/ types.ts（契约）/ ui/（默认对话 UI 三件套）
└─ 依赖方式：core 的 package.json 提交 **git 依赖 + commit id**（如 "github:windwhiterain/comrade-harness-lib#2e0ba22…"，
   **不用 tag**——tag 被 force amend 移动后 bun 不感知，lock 永远锁旧 sha，fork 出的 core 静默装旧代码，真实踩过；
   ...
   本地开发用 gitignored 的 package.local.json 覆盖为 link: 路径依赖：
   `cd cores/standard && bun run local:on`（符号链接到本地 lib，改 lib 即时生效、类型检查走本地代码，
   package.json/bun.lock 自动 skip-worktree 不会被误提交）；`local:off` 还原 git 依赖。
...
- **会话级模型记忆**（2026-08-15）：`sessions` 表带 provider_id/model_id 列（ALTER 迁移）。`GET /api/models?session=` 该会话记过的模型优先，无记录回落 core 启动时的初始模型（`initialModel` 锚点——启动快照，不是被其他会话切换污染的"当前"）；`POST /api/models {providerId, modelId, session}` 带 session 落库、不带只切全局。**新建/分叉会话继承当前会话的设置**（`POST /api/sessions {settingsFrom: <当前会话 id>}`，服务端复制 provider/model 列）——新会话与源会话用同一模型，不回落启动锚点。
...
**两层**：资源（LLM / 记忆 / 工具 / UI / HTTP 壳——参数组合就够了，Claude/Codex 也能"加载资源"）和**数据流**（harness 的灵魂）。

- **节点 = 普通函数**（`comrade-harness-lib/nodes.ts`）：`buildSystemPrompt`（提示词随 core 身份动态生成：standard🔒 强调不可改、fork 出的 core 强调可自改自）/ `loadHistory`（记忆）/ `composeMessages` / `callLLM`（一次补全，失败抛出）/ `runTools`（执行 tool_calls，`done` 收尾）/ `saveHistory`。契约在 `comrade-harness-lib/types.ts`（普通 TS 接口，不是插件 API）。
- **流 = core 的代码**：`cores/standard/src/index.ts` 用普通控制流（顺序 + for + if）把节点串成 harness——**lib 里没有任何隐藏的编排逻辑**，`createHarness({ flow, ...资源 })` 只是 HTTP 壳 + 资源注入。
- **自定义 = 改流**：fork 出 core 后，替换节点 = 换个函数（如把 `callLLM` 换成多模型路由/流式）；插入节点 = 加一行调用（如 LLM 前注入 RAG 上下文、工具结果后加反思）。没有注册表、没有图结构、没有插件 API。
...
- **上下文注入**（路线图第 6 项，随流化落地）：standard 的流默认 `loadHistory(ctx.memory, 8)` 把最近 8 条历史喂给 LLM——每次消息不再是独立回合；想恢复独立回合就改成 `loadHistory(ctx.memory, 0)`。
- **过程步骤行**（2026-08-15）：standard 的流把每轮的思考片段与工具调用（名称/参数/结果）以 `role="step"` 的 JSON 行写进历史（`{type:"think"}` / `{type:"tool"}`），UI 渲染成可折叠卡片；`loadHistory` 跳过 step 行——它们只作展示，不喂给 LLM。删除/截断对 step 行同样有效。思考片段优先取 DeepSeek 推理模型的 `reasoning_content`（llm.ts 已适配；`LLM_MODEL=deepseek-reasoner` 时每轮都有 think 卡片），没有 reasoning 字段的模型（如 deepseek-chat）则取调用工具时返回的 content，通常为空。
...
- **fork 继承数据流**：fork 复制 standard 的 `src/index.ts`（流代码）——自定义 = 改流的代码（替换节点 = 换个函数；插入节点 = 加一行调用），实测：插一个"注入上下文"节点，reload 后行为即变。
...
7. **编排在 core，lib 不藏流程**：harness 的数据流是 core 的 `src/index.ts` 里的普通代码，lib 只提供节点函数和运行时壳——lib 里不允许出现"隐藏的 agent 循环"。
...
14. **standard 的本地依赖覆盖（package.local.json + skip-worktree）**：`bun run local:on` 把 package.json 换成 link: 本地依赖并 skip-worktree（package.json/bun.lock 的本地改动不会被提交）。**git pull / git reset 会静默覆盖这两个文件**（回到 git 依赖版本），重跑 local:on 即可；local:off 还原。package.local.json 是 gitignored 的"不上传的文件"；bun link 的注册文件在 ~/.bun（用户目录，同样不上传）。
...
6. ~~**会话上下文注入**~~ ✅ **已落地（2026-08-14，随数据流化）**：standard 的流默认 `loadHistory(ctx.memory, 8)` 喂最近 8 条历史；想独立回合就改参数。
